    ...
    selectedQuestions.forEach((q, index) => {
        const questionHTML = `
            <div style="margin: 20px; padding: 10px; border-radius: 10px;">
                ...
                =
                <span id="targetCups${index}" style="font-weight: bold;">${q.targetCups}</span>
...
function generateValues() {
    ...
    values.forEach(value => {
        const valueHTML = `
            <td><div draggable="true" ondragstart="drag(event)" id="${value} ounces" style="background-color: #ffcccc; border: 2px solid #333; border-radius: 5px; padding: 5px; cursor: move;">${value} ounces</div></td>
        `;
...
function checkAnswer(index) {
    ...
    const targetCups = selectedQuestions[index].targetCups;

    if (!isNaN(first) && !isNaN(second)) {
        const calculatedCups = ((first + second)/8) + ' cups';
        if (calculatedCups === targetCups) {
            if (!document.getElementById(`result${index}`).innerText) {
                document.getElementById('result').innerText = `Correct! The total number of cups is ${calculatedCups}.☺️`;
